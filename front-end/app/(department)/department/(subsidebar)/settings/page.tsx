...
const SettingsPage = () => {
  ...
  // Use effect to decode token and set employeeName
  useEffect(() => {
  ...
  const handleUpdatePassword = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (employeeName !== username) {
      toast.success("You can only change your own password.");
      return;
    }

    try {
      const response = await fetch("http://localhost:3002/api/registers/updatePassword", {
        method: "POST",
        headers: {
            ...
            <div className="grid grid-cols-8 pt-3 sm:grid-cols-10">
              ...
              <div className="col-span-2 hidden sm:block">
                <ul>
                  <li
                    onClick={() => handleTabClick("theme")}
                    className={`mt-5 cursor-pointer border-l-2 px-2 py-2 font-semibold transition ${activeTab === "theme"
                  ...
                  <li
                    onClick={() => handleTabClick("accounts")}
                    className={`mt-5 cursor-pointer border-l-2 px-2 py-2 font-semibold transition ${activeTab === "accounts"
                      ? "border-l-blue-700 text-blue-700"
                      : "border-transparent"
                      }`}
                  >
                  ...
                  {/* <li
                    onClick={() => handleTabClick("notifications")}
                    className={`mt-5 cursor-pointer border-l-2 px-2 py-2 font-semibold transition ${activeTab === "notifications"
                      ? "border-l-blue-700 text-blue-700"
                      : "border-transparent"
                      }`}
                  >
                ...
                {activeTab === "theme" && (
                  <div className="pt-4">
                    ...
                    <div className="flex items-center justify-between py-4 px-6 bg-gray-100 rounded-lg shadow-md">
                      ...
                      <div className="flex items-center">
                        <span className={`text-gray-600 mr-4 ${isDarkMode ? 'text-gray-400' : ''}`}>Dark</span>
                        <label className="relative inline-flex items-center cursor-pointer">
                          ...
                          />
                          <div className={`w-11 h-6 ${isDarkMode ? 'bg-blue-600' : 'bg-gray-300'} rounded-full transition-colors`}></div>
                          <div
                            className={`absolute w-6 h-6 bg-white border border-gray-300 rounded-full transform transition-transform ${isDarkMode ? 'translate-x-5' : 'translate-x-0'
                              }`}
                          ></div>
                        </label>
                        <span className={`text-gray-600 ml-4 ${isDarkMode ? 'text-gray-100' : ''}`}>Light</span>
                      </div>
                    </div>
